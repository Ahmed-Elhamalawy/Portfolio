...
interface CustomIconProps {
  ...
  nopress?: boolean; // disable pointer events if true
  onPress?: () => void; // click handler
}

const ProjectsIcon: React.FC<CustomIconProps> = ({
  width = 24,
  height = 24,
  color = "currentColor",
  style,
  ...
  return (
    <svg
      ...
      style={{
        cursor: nopress ? "default" : "pointer",
        pointerEvents: nopress ? "none" : "auto",
        ...style,
      }}
      onClick={() => {
        if (!nopress && onPress) onPress();
      }}
    >
      <path
